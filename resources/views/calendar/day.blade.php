<div class="timex-day">
    <div @class([ 'text-gray-400'=> !$isCurrentMonthDay || $isWeekend,
        ...
        <span class="absolute">
            <div @class([ 'ml-1 text-xs' , 'hidden lg:block' ])>
    ...
    <div class="hidden lg:block overflow-x-hidden overflow-y-auto scroll-smooth" style="height: 88px;">
        <div class="grid grid-flow-row gap-0.5" id="{{$timestamp}}" data-status-id="{{$timestamp}}">
            ...
            <div id="{{ $event->getEventID() }}" wire:click="$emitUp('onEventClick','{{$event->getEventID()}}')">
                <livewire:timex-event :body="$event->getBody()" :category="$event->getCategory()"
                    :color="$event->getColor()" :event-i-d="$event->getEventID()" :icon="$event->getIcon()"
                    :is-all-day="$event->getIsAllDay()" :organizer="$event->getOrganizer()" :start="$event->getStart()"
                    :start-time="$event->getStartTime()" :subject="$event->getSubject()" />
            </div>
            @endforeach
        </div>
    </div>
    <div class="flex pl-2 gap-0.5 truncate lg:hidden">
        ...
        <span style="width: 6px; height: 6px" @class([ 'rounded-full' , 'bg-' .$event->getColor().'-500' =>
            $event->getColor() != 'secondary',
